...
async function getContextSources(
    ...
    if (!sources.has(key)) {
      sources.set(key, {
        ...
        sourceType: normalizeSourceType(
          (source as { source_type?: unknown }).source_type,
        ),
  ...
  const sourceTypeByResourceId = new Map(
    data.map((source) => [source.id, normalizeSourceType(source.source_type)]),
  );
  ...
  return selected.slice(0, 6).map(({ source }, index) => ({
    ...
    sourceType:
      source.sourceType ??
      (source.resourceId ? sourceTypeByResourceId.get(source.resourceId) : null),
    id: `source-${index + 1}`,
  }));
}
  ...
  return candidates
    ...
    .map((source, index) => ({
      ...
      sourceType:
        source.sourceType ??
        (source.resourceId
          ? (metadataByResourceId.get(source.resourceId)?.sourceType ?? null)
          : null),
      id: `research-${index + 1}`,
    }));
}
...
function parseRecommendations(
  ...
  researchSourceOptions: RecommendationSourceOption[],
    ...
    return parsed.recommendations
      .map((item, index) => {
        ...
        const sources = sourceIds
          ...
          .slice(0, 2);
        const researchSourceIds = Array.isArray(
          recommendation.researchSourceIds,
        )
          ? recommendation.researchSourceIds.filter(
              (sourceId): sourceId is string =>
                typeof sourceId === "string",
            )
          : [];
        const researchSources = researchSourceIds
          .flatMap((sourceId) => {
            const source = researchSourceOptions.find(
              (option) => option.id === sourceId,
            );
...
async function getResourceContextByTags(
  ...
  const { data, error } = await supabase
    .from("resource_chunks")
    .select(
      "id, resource_id, chunk_text, chunk_summary, topic_tags, use_case_tags, curated_resources(id, title, source_name, source_url, source_type)",
    )
      ...
      input: [
        ...
        },
        {
          ...
          content: JSON.stringify({
            ...
            research_source_options: researchSources.map((source) => ({
              id: source.id,
              title: source.title,
              source_name: source.sourceName,
              source_type: source.sourceType,
